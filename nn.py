...
# Convert text data to numerical data (using character-level encoding)
...
text_to_text_nn = TextToTextNeuralNetwork(
...
# Create and train the text-to-text neural network
text_to_text_nn.train(X_train_indices, y_train_indices, epochs=100, learning_rate=0.01, verbose=True)

# Test inference
X_test_text = ["cat", "dog", "apple", "banana"]
X_test_indices = [np.array([char_to_index[char] for char in word]) for word in X_test_text]

# Pad the test sequences to have the same length as training sequences
...
for i in range(len(X_test_indices)):
    ...
    predicted_words = [index_to_char[index] for index in predicted_sequence]
    print(f"Input: {X_test_text[i]}, Predicted: {''.join(predicted_words)}")
